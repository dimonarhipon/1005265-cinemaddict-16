...
export const createFilmDetails = (movie) => {
  const {title, alternative_title, total_rating, poster, age_rating, director, writers, actors, description, genre, release, runtime} = movie.film_info;
  ...
  const addComments = (count) => {
    return Array.from({length: count}, createComment).join(``);
  };
